...
class SingleLinkage:
    ...
    # calculate between cluster distance using single-linkage algorithm
    def singleDistance(self):
        mini = 1e99
        merge = (None, None)
                ...
                for c in self.clusters[i]:
                    for d in self.clusters[j]:
                        ...
                        if dist < mini:
                            mini = dist
                            merge = (i, j)
            betweenDistance.append(mini)                                            # choose the minimum distance as the distance between two clusters
